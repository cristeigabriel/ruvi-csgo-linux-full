...
bool fgui::element::unlocked() {
  ...
  if (!m_controller || m_page_index == -1) return true;

  switch (m_controller->m_type) {

  case static_cast<int>(fgui::detail::element_type::COMBOBOX):
    return std::reinterpret_pointer_cast<fgui::combobox>(m_controller)
               ->get_index() == m_page_index;

  case static_cast<int>(fgui::detail::element_type::CHECKBOX):
    return std::reinterpret_pointer_cast<fgui::checkbox>(m_controller)
               ->get_bool() == m_page_index;

  case static_cast<int>(fgui::detail::element_type::LISTBOX):
    return std::reinterpret_pointer_cast<fgui::listbox>(m_controller)
               ->get_index() == m_page_index;

  case static_cast<int>(fgui::detail::element_type::MULTIBOX):
    return std::reinterpret_pointer_cast<fgui::multibox>(m_controller)
        ->get_selected(m_page_index);

  case static_cast<int>(fgui::detail::element_type::TAB):
    return std::reinterpret_pointer_cast<fgui::tabs>(m_controller)
               ->get_index() == m_page_index;
  }
...
fgui::point fgui::element::get_absolute_position() {
  ...
  if (!m_parent_element) return {get_position().x, get_position().y};
  ...
  if (m_parent_element->get_family(fgui::detail::element_type::CONTAINER) &&
      std::reinterpret_pointer_cast<fgui::container>(m_parent_element)
          ->get_scrollbar_state())
