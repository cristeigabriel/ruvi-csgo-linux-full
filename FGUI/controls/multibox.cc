...
fgui::multibox::multibox() {
  ...
  fgui::multibox::m_type =
      static_cast<int>(fgui::detail::element_type::MULTIBOX);
...
void fgui::multibox::draw() {
  ...
  fgui::dimension text_size =
      fgui::render.get_text_size(fgui::multibox::get_font(), m_title);

  // multibox body
  fgui::render.outline(area.left, area.top, area.right, area.bottom,
                       fgui::color(style.multibox.at(0)));

  if (fgui::input_system::mouse_in_area(area))
    fgui::render.outline(area.left + 2, area.top + 2, area.right - 4,
                         area.bottom - 4, fgui::color(style.multibox.at(3)));
  else
    fgui::render.outline(area.left + 1, area.top + 1, area.right - 2,
                         area.bottom - 2, fgui::color(style.multibox.at(2)));

  fgui::render.colored_gradient(area.left + 3, area.top + 3, area.right - 6,
                                area.bottom - 6,
                                fgui::color(style.multibox.at(1)),
                                fgui::color(style.multibox.at(2)), false);

  // lines
  fgui::render.rect(area.left + (area.right - 20),
                    (area.top + (area.bottom / 2) - 3) + 1, 10, 1,
                    fgui::color(style.text.at(0)));
  fgui::render.rect(area.left + (area.right - 20),
                    (area.top + (area.bottom / 2) - 3) + 3, 10, 1,
                    fgui::color(style.text.at(0)));
  fgui::render.rect(area.left + (area.right - 20),
                    (area.top + (area.bottom / 2) - 3) + 5, 10, 1,
                    fgui::color(style.text.at(0)));

  // multibox label
  fgui::render.text(area.left, (area.top - text_size.height) - 2,
                    fgui::color(style.text.at(0)), fgui::multibox::get_font(),
                    m_title);
  ...
  if (format.length() <= 0) format += "None";

  // draw the current selected items
  fgui::render.text(
      area.left + 5, area.top + (area.bottom / 2) - (text_size.height / 2),
      fgui::color(style.text.at(0)), fgui::multibox::get_font(), format);
  ...
  if (m_opened) {
    ...
    fgui::render.outline(area.left - 1, (area.top + 23) - 1, area.right + 2,
                         ...
                         fgui::color(style.multibox.at(3)));
    ...
    for (std::size_t i = 0; i < m_info.size(); i++) {
      ...
      fgui::rect item_area = {a.x,
                              a.y + 23 + (m_item_height * static_cast<int>(i)),
                              m_width, m_item_height};
