...
public:
  ...
  inline fgui::point get_position() const noexcept { return {m_x, m_y}; }
  ...
  inline std::string_view get_title() const noexcept { return m_title; }
  ...
  inline bool get_flag(const fgui::detail::element_flags &flag) const noexcept {
    ...
    if (m_flags & static_cast<int>(flag)) return true;
  ...
  inline bool get_family(const fgui::detail::element_type &type) const
      noexcept {

    if (m_type == static_cast<int>(type)) return true;
  ...
  inline void
  set_controller(const std::shared_ptr<fgui::element> &controller) noexcept {

    m_controller = controller;
  }

  // sets a custom indentificator for the element (this is used by the config
  // system)
  ...
  inline void set_font(const std::string_view font_family, int size,
                       int flags = 0, bool bold = false) noexcept {
  ...
  inline void set_font(const fgui::element_font &element_font) noexcept {
    ...
    fgui::render.create_font(m_font, element_font.family, element_font.size,
                             element_font.flags, element_font.bold);
  }

  // returns the font that the element is currently using
  inline fgui::font get_font() const noexcept { return m_font; }

protected:
  virtual void draw() = 0; // draw the element
  virtual void
               handle_input() = 0; // handles the element keyboard and mouse input
  virtual void update()       = 0; // handles the element updates
  virtual void tooltip()      = 0; // draw the element tooltip
  virtual void
               save(nlohmann::json &json_module) = 0; // write the element state on a file
  virtual void load(
      const std::string_view file_name) = 0; // read the element state of a file
protected:
  int                            m_type;
  int                            m_flags;
  int                            m_x, m_y;
