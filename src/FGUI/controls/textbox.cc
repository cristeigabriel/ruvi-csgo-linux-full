...
void fgui::textbox::draw() {
  ...
  fgui::dimension text_size =
      fgui::render.get_text_size(fgui::textbox::get_font(), m_title);

  // textbox body
  fgui::render.outline(area.left, area.top, area.right, area.bottom,
                       fgui::color(style.textbox.at(0)));

  if (fgui::input_system::mouse_in_area(area) || m_is_getting_key)
    fgui::render.outline(area.left + 2, area.top + 2, area.right - 4,
                         area.bottom - 4, fgui::color(style.textbox.at(3)));
  else
    fgui::render.outline(area.left + 1, area.top + 1, area.right - 2,
                         area.bottom - 2, fgui::color(style.textbox.at(2)));

  fgui::render.colored_gradient(area.left + 3, area.top + 3, area.right - 6,
                                area.bottom - 6,
                                fgui::color(style.textbox.at(1)),
                                fgui::color(style.textbox.at(2)), false);

  // textbox label
  fgui::render.text(area.left, (area.top - text_size.height) - 2,
                    fgui::color(style.text.at(0)), fgui::textbox::get_font(),
                    m_title);

  if (m_text.empty()) return;

  // typed text size
  fgui::dimension typed_text_size =
      fgui::render.get_text_size(fgui::textbox::get_font(), m_text);

  // draw custom text
  if (m_text_flag & static_cast<int>(fgui::text_flags::SECRET))
    fgui::render.text(area.left + 5,
                      area.top + (area.bottom / 2) -
                          (typed_text_size.height / 2) - 1,
                      fgui::color(style.text.at(0)), fgui::textbox::get_font(),
    ...
    std::transform(upper_case_display.begin(), upper_case_display.end(),
                   upper_case_display.begin(), ::toupper);
    ...
    fgui::render.text(
        area.left + 5,
        area.top + (area.bottom / 2) - (typed_text_size.height / 2) - 1,
        fgui::color(style.text.at(0)), fgui::textbox::get_font(), m_text);
  ...
  if (m_is_getting_key) {
    ...
    // caret size
    fgui::dimension caret_text_size =
        fgui::render.get_text_size(fgui::textbox::get_font(), caret);

    // draw the caret
    fgui::render.line(area.left + caret_text_size.width + 7, area.top + 5,
                      area.left + caret_text_size.width + 7,
  ...
  if (m_text_selected)
    fgui::render.rect(area.left + 5,
                      area.top + (area.bottom / 2) -
                          (typed_text_size.height / 2) - 1,
                      typed_text_size.width, typed_text_size.height,
                      fgui::color(style.textbox.at(3), 100));
}
...
void fgui::textbox::update() {
  ...
  if (m_is_getting_key) {
    ...
    for (std::size_t i = 0; i < 256 /* KEY_MAX */; i++) {
      ...
      if (!fgui::input_system::key_press(i)) continue;
      ...
      if (fgui::input_system::key_held(fgui::external::KEY_LSHIFT) ||
          fgui::input_system::key_held(fgui::external::KEY_RSHIFT) ||
          m_text_flag & static_cast<int>(fgui::text_flags::UPPERCASE))
    ...
    if (m_text.length() > 0) {
      ...
      if (fgui::input_system::key_press(fgui::external::KEY_BACKSPACE) &&
          m_text_input_pos > 0 && !m_text_selected) {
      ...
      else if (fgui::input_system::key_held(fgui::external::KEY_BACKSPACE) &&
               m_text_input_pos > 0 && !m_text_selected) {
      ...
      else if (fgui::input_system::key_held(fgui::external::KEY_LCONTROL) &&
               fgui::input_system::key_press(fgui::external::KEY_A)) {
      ...
      if (fgui::input_system::key_press(fgui::external::KEY_LEFT) &&
          m_text_input_pos > 0)
        m_text_input_pos--;

      if (fgui::input_system::key_press(fgui::external::KEY_RIGHT) &&
          m_text_input_pos < m_text.length())
        m_text_input_pos++;
    }
  }
}
