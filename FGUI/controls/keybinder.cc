...
fgui::keybinder::keybinder() {
  ...
  fgui::keybinder::m_type =
      static_cast<int>(fgui::detail::element_type::KEYBINDER);
...
void fgui::keybinder::draw() {
  ...
  fgui::dimension text_size =
      fgui::render.get_text_size(fgui::keybinder::get_font(), m_title);

  // keybinder body
  fgui::render.outline(area.left, area.top, area.right, area.bottom,
                       fgui::color(style.keybinder.at(0)));

  if (fgui::input_system::mouse_in_area(area) || m_is_getting_key)
    fgui::render.outline(area.left + 2, area.top + 2, area.right - 4,
                         area.bottom - 4, fgui::color(style.keybinder.at(3)));
  else
    fgui::render.outline(area.left + 1, area.top + 1, area.right - 2,
                         area.bottom - 2, fgui::color(style.keybinder.at(2)));

  fgui::render.colored_gradient(area.left + 3, area.top + 3, area.right - 6,
                                area.bottom - 6,
                                fgui::color(style.keybinder.at(1)),
                                fgui::color(style.keybinder.at(2)), false);

  // keybinder label
  fgui::render.text(area.left, (area.top - text_size.height) - 2,
                    fgui::color(style.text.at(0)), fgui::keybinder::get_font(),
                    m_title);

  // bound key text size
  fgui::dimension bound_key_text_size = fgui::render.get_text_size(
      fgui::keybinder::get_font(),
      m_key > 0 ? m_iinput_system[m_key].data() : "Not Bound");

  // draw the current key
  fgui::render.text(area.left + (area.right / 2) -
                        (bound_key_text_size.width / 2),
                    area.top + (area.bottom / 2) - (text_size.height / 2) - 1,
                    fgui::color(style.text.at(0)), fgui::keybinder::get_font(),
                    m_key > 0 ? m_iinput_system[m_key].data() : "Not Bound");
}
  ...
  if (m_tooltip.length() > 0) {
    ...
    if (fgui::input_system::mouse_in_area(area)) {
      fgui::render.rect(cursor.x + 10, cursor.y + 20,
                        ...
                        fgui::color(style.keybinder.at(3)));
      fgui::render.text(
