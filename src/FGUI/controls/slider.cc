...
void fgui::slider::draw() {
  ...
  fgui::dimension text_size =
      fgui::render.get_text_size(fgui::slider::get_font(), m_title);

  // slider body
  fgui::render.outline(area.left, area.top, area.right, area.bottom,
                       fgui::color(style.slider.at(0)));
  fgui::render.colored_gradient(
      area.left + 1, area.top + 1, area.right - 2, area.bottom - 2,
      fgui::color(style.slider.at(1)), fgui::color(style.slider.at(2)), false);

  if (fgui::input_system::mouse_in_area(area) || m_dragging)
    fgui::render.outline(area.left + 1, area.top + 1, location - 2,
                         area.bottom - 2, fgui::color(style.slider.at(3)));
  else
    fgui::render.outline(area.left + 1, area.top + 1, area.right - 2,
                         area.bottom - 2, fgui::color(style.slider.at(2), 150));

  fgui::render.colored_gradient(
      area.left + 2, area.top + 2, location - 4, area.bottom - 4,
      fgui::color(style.slider.at(3)), fgui::color(style.slider.at(4)), false);

  // slider label
  fgui::render.text(area.left, (area.top - text_size.height) - 2,
                    fgui::color(style.text.at(0)), fgui::slider::get_font(),
                    m_title);

  // slider custom text value
  if (m_min_text.length() > 0) {

    if (m_value <= m_min) custom_value = m_min_text;
  }

  else if (m_max_text.length() > 0) {

    if (m_value >= m_max) custom_value = m_max_text;
  }
  ...
  fgui::dimension slider_value_text_size =
      fgui::render.get_text_size(fgui::slider::get_font(), custom_value);

  // slider value
  fgui::render.text((area.left + area.right) - slider_value_text_size.width,
                    (area.top - text_size.height) - 2,
                    fgui::color(style.text.at(0)), fgui::slider::get_font(),
                    custom_value.data());
}
...
void fgui::slider::handle_input() {
  ...
  if (fgui::input_system::mouse_in_area(area)) m_dragging = true;
}
...
void fgui::slider::update() {
  ...
  if (fgui::input_system::mouse_in_area(area)) {
    ...
    if (fgui::input_system::key_held(fgui::external::KEY_RIGHT) ||
        fgui::input_system::key_held(fgui::external::KEY_UP)) {
      m_value += 1.f;
    ...
    else if (fgui::input_system::key_held(fgui::external::KEY_LEFT) ||
             fgui::input_system::key_held(fgui::external::KEY_DOWN)) {
      m_value -= 1.f;
