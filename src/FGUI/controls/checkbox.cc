...
fgui::checkbox::checkbox() {
  ...
  fgui::checkbox::m_original_width = fgui::checkbox::m_width;
  fgui::checkbox::m_type =
      static_cast<int>(fgui::detail::element_type::CHECKBOX);
...
void fgui::checkbox::draw() {
  ...
  fgui::render.outline(area.left, area.top, area.right, area.bottom,
                       fgui::color(style.checkbox.at(0)));
  fgui::render.colored_gradient(area.left + 1, area.top + 1, area.right - 2,
                                area.bottom - 2,
                                fgui::color(style.checkbox.at(1)),
                                fgui::color(style.checkbox.at(2)), false);

  if (fgui::input_system::mouse_in_area(area) || m_checked)
    fgui::render.outline(area.left + 2, area.top + 2, area.right - 4,
                         area.bottom - 4, fgui::color(style.checkbox.at(3)));

  if (m_checked)
    fgui::render.colored_gradient(area.left + 3, area.top + 3, area.right - 6,
                                  area.bottom - 6,
                                  fgui::color(style.checkbox.at(3)),
                                  fgui::color(style.checkbox.at(4)), false);

  // checkbox label
  fgui::render.text(area.left + area.right + 5, area.top,
                    fgui::color(style.text.at(0)), fgui::checkbox::get_font(),
                    m_title);
}
...
void fgui::checkbox::handle_input() {
  ...
  // switch
  if (fgui::input_system::mouse_in_area(area)) m_checked = !m_checked;
}
...
void fgui::checkbox::update() {
  ...
  fgui::dimension text_size =
      fgui::render.get_text_size(fgui::checkbox::get_font(), m_title);
