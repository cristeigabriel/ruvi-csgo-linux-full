...
public:
  ...
  static fgui::color hsb_to_rgb(float hue, float saturation, float brightness,
                                int alpha = 255) {
  ...
  static fgui::color interpolate(fgui::color color1, fgui::color color2,
                                 float ttt) {

    float r = (std::clamp<int>(color1.m_red, 0, 255) * (1.f - ttt) +
               std::clamp<int>(color2.m_red, 0, 255) * ttt);
    float g = (std::clamp<int>(color1.m_green, 0, 255) * (1.f - ttt) +
               std::clamp<int>(color2.m_green, 0, 255) * ttt);
    float b = (std::clamp<int>(color1.m_blue, 0, 255) * (1.f - ttt) +
               std::clamp<int>(color2.m_blue, 0, 255) * ttt);
    float a = (std::clamp<int>(color1.m_alpha, 0, 255) * (1.f - ttt) +
               std::clamp<int>(color2.m_alpha, 0, 255) * ttt);
