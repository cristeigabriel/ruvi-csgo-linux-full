...
void fgui::notification::draw() {
  ...
  for (std::size_t i = 0; i < m_info.size(); i++) {
    ...
    fgui::dimension text_size = fgui::render.get_text_size(
        fgui::notification::get_font(), m_info[i].text);
    ...
    if (m_info[i].animation_type == fgui::animation_type::LINEAR) {
      ...
      area = {m_x + (screen_width - m_info[i].animation_progress),
              m_y + (m_height * static_cast<int>(i)), m_width + 30, m_height};

      // notification body
      fgui::render.outline(area.left, area.top, area.right, area.bottom,
                           fgui::color(style.notifications.at(0), 235));
      fgui::render.rect(area.left + 1, area.top + 1, 1, area.bottom - 2,
                        fgui::color(style.notifications.at(3)));
      fgui::render.rect(area.left + 2, area.top + 2, area.right - 4,
                        area.bottom - 4,
                        fgui::color(style.notifications.at(1), 235));

      // notification text
      if (m_info[i].text.length() > 32)
        fgui::render.text(area.left + 10,
                          area.top + (area.bottom / 2) - (text_size.height / 2),
                          fgui::color(style.text.at(0), 235),
                          fgui::notification::get_font(),
                          m_info[i].text.replace(m_info[i].text.begin() + 32,
                                                 m_info[i].text.end(), "..."));
      else
        fgui::render.text(area.left + 10,
                          area.top + (area.bottom / 2) - (text_size.height / 2),
      ...
      area = {m_x + (screen_width - m_width),
              m_y + (m_height * static_cast<int>(i)), m_width + 30, m_height};

      // notification body
      fgui::render.outline(
          area.left, area.top, area.right, area.bottom,
          fgui::color(style.notifications.at(0), m_info[i].animation_progress));
      fgui::render.rect(
          area.left + 1, area.top + 1, 1, area.bottom - 2,
          fgui::color(style.notifications.at(3), m_info[i].animation_progress));
      fgui::render.rect(
          area.left + 2, area.top + 2, area.right - 4, area.bottom - 4,
          fgui::color(style.notifications.at(1), m_info[i].animation_progress));
      ...
      if (m_info[i].text.length() > 32)
        fgui::render.text(
            area.left + 10,
            area.top + (area.bottom / 2) - (text_size.height / 2),
            fgui::color(style.text.at(0), m_info[i].animation_progress),
            fgui::notification::get_font(),
            m_info[i].text.replace(m_info[i].text.begin() + 32,
                                   m_info[i].text.end(), "..."));
      else
  ...
  for (std::size_t i = 0; i < m_info.size(); i++) {
    ...
    if (!m_info[i].state) {
      ...
      if (m_info[i].animation_type == fgui::animation_type::LINEAR) {
        ...
        if (m_info[i].tick < m_width) m_info[i].animation_progress++;
        ...
        if (m_info[i].tick >= m_width + m_stay_time) {
          ...
          if (m_info[i].animation_progress == 0) m_info[i].state = false;
        }
      }
      ...
      // fade animation
        ...
        if (m_info[i].tick < alpha_value) m_info[i].animation_progress++;
