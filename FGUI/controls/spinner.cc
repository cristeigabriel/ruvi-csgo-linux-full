...
void fgui::spinner::draw() {
  ...
  fgui::dimension text_size =
      fgui::render.get_text_size(fgui::spinner::get_font(), m_title);

  // spinner body
  fgui::render.outline(area.left, area.top, area.right, area.bottom,
                       fgui::color(style.spinner.at(0)));

  if (fgui::input_system::mouse_in_area(area))
    fgui::render.outline(area.left + 2, area.top + 2, area.right - 4,
                         area.bottom - 4, fgui::color(style.spinner.at(3)));
  else
    fgui::render.outline(area.left + 1, area.top + 1, area.right - 2,
                         area.bottom - 2, fgui::color(style.spinner.at(2)));

  fgui::render.rect(area.left + 3, area.top + 3, area.right - 6,
                    area.bottom - 6, fgui::color(style.spinner.at(1)));
  fgui::render.rect((area.left + 3) + (area.right - 15), area.top + 3, 15 - 5,
                    area.bottom - 6, fgui::color(style.spinner.at(2)));

  // keybinder label
  fgui::render.text(area.left, (area.top - text_size.height) - 2,
                    fgui::color(style.text.at(0)), fgui::spinner::get_font(),
                    m_title);

  // spinner value text size
  fgui::dimension value_text_size = fgui::render.get_text_size(
      fgui::spinner::get_font(), std::to_string(m_value));

  // current value
  fgui::render.text(area.left + 10,
                    area.top + (area.bottom / 2) - (value_text_size.height / 2),
                    fgui::color(style.text.at(0)), fgui::spinner::get_font(),
  ...
  // up down
  fgui::render.rect(up_button_area.left - 3,
                    up_button_area.top + (up_button_area.bottom / 2), 5, 1,
                    fgui::color(up_color));
  fgui::render.rect(up_button_area.left - 2,
                    up_button_area.top + (up_button_area.bottom / 2) - 1, 3, 1,
                    fgui::color(up_color));
  fgui::render.rect(up_button_area.left - 1,
                    up_button_area.top + (up_button_area.bottom / 2) - 2, 1, 1,
                    fgui::color(up_color));

  // down button
  fgui::render.rect(down_button_area.left - 3,
                    down_button_area.top + (down_button_area.bottom / 2), 5, 1,
                    fgui::color(down_color));
  fgui::render.rect(down_button_area.left - 2,
                    down_button_area.top + (down_button_area.bottom / 2) + 1, 3,
                    1, fgui::color(down_color));
  fgui::render.rect(down_button_area.left - 1,
                    down_button_area.top + (down_button_area.bottom / 2) + 2, 1,
                    1, fgui::color(down_color));
}
