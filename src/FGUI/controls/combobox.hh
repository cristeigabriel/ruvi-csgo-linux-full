...
public:
  ...
  inline void clear_items() noexcept { m_info.clear(); }
  ...
  inline void set_index(int index) noexcept { m_index = index; }
  ...
  // sets a custom state for the combobox (if the dropdown list is opened or
  // not)
  inline void set_state(fgui::state state) { m_opened = state; }

  // returns the current state of the combobox (if the dropdown list is opened
  // or not)
