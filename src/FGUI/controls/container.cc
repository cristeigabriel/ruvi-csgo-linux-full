...
fgui::container::container() {
  ...
  fgui::container::m_type =
      static_cast<int>(fgui::detail::element_type::CONTAINER);
...
void fgui::container::draw() {
  ...
  fgui::dimension text_size =
      fgui::render.get_text_size(fgui::container::get_font(), m_title);

  // if the container is not bound to another element it will behave like a
  // window
  if (!m_parent_element) {

    // container window header
    fgui::render.outline(a.x, a.y, m_width, m_height,
                         fgui::color(style.window.at(0), 150));
    fgui::render.outline(a.x + 1, a.y + 1, m_width - 2, m_height - 2,
                         fgui::color(style.window.at(2)));
    fgui::render.colored_gradient(a.x + 2, a.y + 2, m_width - 4, 25,
                                  fgui::color(style.window.at(3), 150),
                                  fgui::color(style.window.at(4), 150), false);
    fgui::render.rect(a.x + 2, (a.y + 2) + 25, m_width - 4, (m_height - 25) - 4,
                      fgui::color(style.window.at(4), 235));
    fgui::render.text(a.x + 10, a.y + (text_size.height / 2),
                      fgui::color(style.text.at(3)),
                      fgui::container::get_font(), m_title);

    // container window background
    fgui::render.outline(a.x + 5, a.y + 25, m_width - 10, (m_height - 25) - 5,
                         fgui::color(style.window.at(2), 150));
    fgui::render.rect((a.x + 5) + 1, (a.y + 25) + 1, (m_width - 10) - 2,
                      (m_height - 25 - 5) - 2, fgui::color(style.window.at(1)));
  ...
  // if the container is bound to a parent element it will behave like a
  // groupbox
    ...
    // container body
    fgui::render.line(a.x - 1, a.y, a.x + 5, a.y,
                      fgui::color(conditional_color));
    fgui::render.line(a.x + 5 + (text_size.width + 10), a.y, a.x + m_width, a.y,
                      fgui::color(conditional_color));
    fgui::render.line(a.x + m_width, a.y, a.x + m_width, a.y + m_height,
                      fgui::color(conditional_color));
    fgui::render.line(a.x, a.y + m_height, a.x + m_width, a.y + m_height,
                      fgui::color(conditional_color));
    fgui::render.line(a.x - 1, a.y, a.x - 1, a.y + m_height,
                      fgui::color(conditional_color));
    ...
    if (m_resizeable) {
      render.rect(a.x + m_width - 5, a.y + m_height - 1, 5, 1,
                  fgui::color(conditional_color));
      render.rect(a.x + m_width - 4, a.y + m_height - 2, 4, 1,
                  fgui::color(conditional_color));
      render.rect(a.x + m_width - 3, a.y + m_height - 3, 3, 1,
                  fgui::color(conditional_color));
      render.rect(a.x + m_width - 2, a.y + m_height - 4, 2, 1,
                  fgui::color(conditional_color));
      render.rect(a.x + m_width - 1, a.y + m_height - 5, 1, 1,
                  fgui::color(conditional_color));
    }
    ...
    if (!m_focused_element) {
      ...
      fgui::rect parent_area = {std::max(a.x, pa.x), std::max(a.y, pa.y),
                                std::min(m_width, m_parent_element->m_width),
                                std::min(m_height, m_parent_element->m_height)};

      fgui::render.clip_rect(parent_area.left, parent_area.top + 1,
                             parent_area.right, parent_area.bottom - 1);
    }
  }
  ...
  for (std::shared_ptr<fgui::element> element : m_elements) {
    ...
    if (element && element->unlocked() &&
        element->get_flag(fgui::detail::element_flags::DRAWABLE)) {

      // lets not draw the focused element here
      if (element == m_focused_element) continue;
      ...
      if (m_scrollable) {
        ...
        /*
         *
         * You can adjust the elements positions here in case your scrolling is
         * wrong
         *
         */

        // check if the elements are inside the container
        if (a.y <= get_absolute_position().y + m_height &&
            a.y + element->get_size().height >= get_absolute_position().y) {
  ...
  if (m_parent_element) {
    ...
    fgui::render.clip_rect(a.x, a.y, get_window()->m_width,
                           get_window()->m_height);
  }
  ...
  if (get_window() == shared_from_this() && m_focused_element &&
      m_focused_element->unlocked() &&
      m_focused_element->get_flag(fgui::detail::element_flags::DRAWABLE)) {
    ...
    if (m_scrollable) {
      ...
      // check if the elements are inside the container
      if (a.y <= get_absolute_position().y + m_height &&
          a.y + m_focused_element->get_size().height >=
              get_absolute_position().y) {
  ...
  if (m_scrollable) {
    ...
    if (m_elements.size() > 75)
      fgui::render.rect((a.x + m_width) - 8 + (8 / 2) - (5 / 2),
                        (a.y + 5) + calculated_position + 4, 5, 10,
                        fgui::color(style.container.at(2)));
    else
      fgui::render.rect((a.x + m_width) - 8 + (8 / 2) - (5 / 2),
                        (a.y + 5) + calculated_position + 4, 5,
                        calculated_size - 27,
                        fgui::color(style.container.at(2)));

    // up button
    fgui::render.rect((a.x + m_width) - 8 + (8 / 2) - (5 / 2), (a.y + 2), 5, 5,
                      fgui::color(style.container.at(2)));

    // down button
    fgui::render.rect((a.x + m_width) - 8 + (8 / 2) - (5 / 2),
                      (a.y + 2) + (m_height - 8), 5, 5,
                      fgui::color(style.container.at(2)));
  }
  ...
  if (m_parent_element && m_hideable) {
    ...
    // 'hidden' text size
    fgui::dimension hidden_text_size =
        fgui::render.get_text_size(fgui::container::get_font(), "Hidden");
    ...
    if (!fgui::input_system::mouse_in_area(container_area)) {
      ...
      if (m_focused_element) m_focused_element.reset();

      // 'hidden' body
      fgui::render.outline(a.x, a.y, m_width, m_height,
                           fgui::color(style.container.at(0)));
      fgui::render.outline(a.x + 2, a.y + 2, m_width - 4, m_height - 4,
                           fgui::color(style.container.at(2)));
      fgui::render.rect(a.x + 3, a.y + 3, m_width - 6, m_height - 6,
                        fgui::color(style.container.at(1)));

      // 'hidden' label
      fgui::render.text(a.x + (m_width / 2) - (hidden_text_size.width / 2),
                        a.y + (m_height / 2) - (hidden_text_size.height / 2),
                        fgui::color(style.text.at(0)),
...
//---------------------------------------------------------
void fgui::container::add_control(const std::shared_ptr<fgui::element> &control,
                                  int page_index, bool manual_size) {
  ...
  if (!manual_size) {
    ...
    if (m_scrollable)
      control->set_size(m_width - (control->get_position().x * 2) -
                            scrollbar_width,
  ...
  // this might look a bit off due to the fact that pixels are counted from the
  // top
  if (m_bottom_element_pos <
      control->get_position().y + control->get_size().height)
    m_bottom_element_pos =
        control->get_position().y + control->get_size().height;
...
void fgui::container::update() {
  ...
  if (m_focused_element && m_focused_element->unlocked()) {
    ...
    fgui::rect focused_control_area = {a.x, a.y, m_focused_element->m_width,
                                       m_focused_element->m_height};
    ...
    if (m_focused_element->get_flag(fgui::detail::element_flags::CLICKABLE) &&
        fgui::input_system::mouse_in_area(focused_control_area) &&
        fgui::input_system::key_press(fgui::external::MOUSE_LEFT)) {

      if (m_scrollable) {

        if (m_focused_element->get_absolute_position().y +
                    m_focused_element->get_size().height <=
                get_absolute_position().y + get_size().height &&
            m_focused_element->get_absolute_position().y >=
                get_absolute_position().y) {
    ...
    if (m_focused_element &&
        m_focused_element->get_flag(fgui::detail::element_flags::CLICKABLE) &&
        fgui::input_system::mouse_in_area(focused_control_area))
  ...
  for (std::shared_ptr<fgui::element> element : m_elements) {
    ...
    if (element->unlocked()) {
      ...
      // if the element can receive input
      if (element->get_flag(fgui::detail::element_flags::CLICKABLE) &&
          fgui::input_system::mouse_in_area(control_area) &&
          fgui::input_system::key_press(fgui::external::MOUSE_LEFT) &&
          !focused_element_clicked) {

        if (m_scrollable) {

          if (element->get_absolute_position().y + element->get_size().height <=
                  get_absolute_position().y + get_size().height &&
              element->get_absolute_position().y >= get_absolute_position().y) {
      ...
      // update the cursor if the element is clickable
      if (element &&
          element->get_flag(fgui::detail::element_flags::CLICKABLE) &&
          fgui::input_system::mouse_in_area(control_area))
        handler::set_cursor(fgui::cursor_type::HAND);
      // else
      //	handler::set_cursor(fgui::cursor_type::ARROW);
  ...
  if (m_scrollable) {
    ...
    m_scroll_offset =
        std::clamp(m_scroll_offset, 0,
                   std::max<int>(0, m_bottom_element_pos - (m_height - 15)));
  }
  ...
  if (m_resizeable) {
    ...
    fgui::rect resizeable_area = {a.x + m_width - 10, a.y + m_height - 10, 15,
                                  15};
